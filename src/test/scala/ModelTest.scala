...
class ModelTest extends FunSuite {
  ...
  test("all possible moves for Queen on board 3x3") {
    val p = new Queen()
    assert(p.getMoves(0, 0, 3, 3) === List((1,0), (2,0), (0,1), (0,2), (2,2), (1,1)))
    assert(p.getMoves(1, 1, 3, 3) === List((0,1), (2,1), (1,0), (1,2), (2,0), (0,2), (2,2), (0,0)))
    assert(p.getMoves(2, 2, 3, 3) === List((0,2), (1,2), (2,0), (2,1), (0,0), (1,1)))
    assert(p.getMoves(2, 0, 3, 3) === List((0,0), (1,0), (2,1), (2,2), (0,2), (1,1)))
  }

  test("all possible moves for Bishop on board 3x3") {
    val p = new Bishop()
    assert(p.getMoves(0, 0, 3, 3) === List((2,2), (1,1)))
    assert(p.getMoves(1, 1, 3, 3) === List((2,0), (0,2), (2,2), (0,0)))
    assert(p.getMoves(2, 2, 3, 3) === List((0,0), (1,1)))
    assert(p.getMoves(0, 2, 3, 3) === List((2,0), (1,1)))
  }

  test("all possible moves for Knight on board 4x4") {
    val p = new Knight()
    assert(p.getMoves(0, 0, 4, 4) === List((2,1), (1,2)))
    assert(p.getMoves(3, 3, 4, 4) === List((2,1), (1,2)))
    assert(p.getMoves(0, 3, 4, 4) === List((2,2), (1,1)))
    assert(p.getMoves(3, 0, 4, 4) === List((2,2), (1,1)))
  }

  test("all possible moves for Knight on board 5x5") {
    val p = new Knight()
    assert(p.getMoves(2, 2, 5, 5) === List((4,1), (4,3), (3,0), (1,0), (3,4), (1,4), (0,1), (0,3)))
  }
}
